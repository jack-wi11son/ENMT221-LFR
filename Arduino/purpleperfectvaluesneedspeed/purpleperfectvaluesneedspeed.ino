...
void PID(int error) {
  ...
  int leftSpeed = constrain(65 - power_difference, 0, 65);
  int rightSpeed = constrain(65 + power_difference, 0, 65);
  ...
  if(update) {
    ...
    if (lastDir == 1) {
      bang = true;
      analogWrite(leftMotorPin, 44);
      analogWrite(rightMotorPin, 0);

    } else if (lastDir == -1) {
      bang = true;
      analogWrite(leftMotorPin, 0);
      analogWrite(rightMotorPin, 44);
    } 
  }
